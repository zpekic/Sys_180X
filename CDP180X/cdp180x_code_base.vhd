...
--
-- L0010.bus_state: .valfield 4 values exec_nop, exec_memread, exec_memwrite, exec_ioread, exec_iowrite, dma_memread, dma_memwrite, int_nop, fetch_memread, -, -, -, -, -, -, - default exec_nop
--
...
--
-- L0029.seq_cond: .if 4 values true, mode_1805, sync, cond_3X, cond_4, cond_5, continue, continue_sw, cond_8, externalInt, counterInt, alu16_zero, cond_CX, traceEnabled, traceReady, false default true
--
...
--
-- L0047.seq_then: .then 8 values next, repeat, return, fork, @ default next
--
...
-- include '.controller <filename.vhd>, <stackdepth>;' in .mcc file to generate pre-canned microcode control unit and connect 'then' to cpu_seq_then

--
-- L0048.seq_else: .else 8 values next, repeat, return, fork, 0x00..0xFF, @ default next
--
...
-- Jump targets allowed!
-- include '.controller <filename.vhd>, <stackdepth>;' in .mcc file to generate pre-canned microcode control unit and connect 'else' to cpu_seq_else

--
-- L0051.reg_d: .regfield 3 values same, alu_y, shift_dn_df, shift_dn_0 default same
--
...
---- Start boilerplate code (use with utmost caution!)
-- update_reg_d: process(clk, cpu_reg_d)
-- begin
...
--
-- L0052.reg_df: .regfield 2 values same, d_msb, d_lsb, alu_cout default same
--
...
---- Start boilerplate code (use with utmost caution!)
-- update_reg_df: process(clk, cpu_reg_df)
-- begin
--	if (rising_edge(clk)) then
--		case cpu_reg_df is
----			when reg_df_same =>
...
--
-- L0053.reg_t: .regfield 2 values same, alu_y, xp, in default same
--
...
---- Start boilerplate code (use with utmost caution!)
-- update_reg_t: process(clk, cpu_reg_t)
-- begin
--	if (rising_edge(clk)) then
--		case cpu_reg_t is
----			when reg_t_same =>
...
--
-- L0054.reg_b: .regfield 2 values same, alu_y, t, df_d_dn default same
--
...
---- Start boilerplate code (use with utmost caution!)
-- update_reg_b: process(clk, cpu_reg_b)
-- begin
--	if (rising_edge(clk)) then
--		case cpu_reg_b is
----			when reg_b_same =>
...
--
-- L0055.reg_x: .regfield 2 values same, n, alu_yhi, p default same
--
...
---- Start boilerplate code (use with utmost caution!)
-- update_reg_x: process(clk, cpu_reg_x)
-- begin
--	if (rising_edge(clk)) then
--		case cpu_reg_x is
----			when reg_x_same =>
...
--
-- L0056.reg_p: .regfield 2 values same, n, alu_ylo, default same
--
...
-- Value "11" allowed
---- Start boilerplate code (use with utmost caution!)
-- update_reg_p: process(clk, cpu_reg_p)
-- begin
--	if (rising_edge(clk)) then
--		case cpu_reg_p is
----			when reg_p_same =>
...
--
-- L0057.reg_in: .regfield 1 values same, alu_y default same
--
...
--
-- L0058.reg_q: .regfield 2 values same, zero, one default same
--
...
---- Start boilerplate code (use with utmost caution!)
-- update_reg_q: process(clk, cpu_reg_q)
-- begin
--	if (rising_edge(clk)) then
--		case cpu_reg_q is
----			when reg_q_same =>
...
--
-- L0059.reg_mie: .regfield 2 values same, enable, disable default same
--
...
---- Start boilerplate code (use with utmost caution!)
-- update_reg_mie: process(clk, cpu_reg_mie)
-- begin
--	if (rising_edge(clk)) then
--		case cpu_reg_mie is
----			when reg_mie_same =>
...
--
-- L0062.reg_trace: .regfield 2 values same, ss_enable_zero, ss_disable_zero, ss_disable_char default same
--
...
---- Start boilerplate code (use with utmost caution!)
-- update_reg_trace: process(clk, cpu_reg_trace)
-- begin
--	if (rising_edge(clk)) then
--		case cpu_reg_trace is
----			when reg_trace_same =>
----				reg_trace <= reg_trace;
--			when reg_trace_ss_enable_zero =>
--				reg_trace <= ss_enable_zero;
--			when reg_trace_ss_disable_zero =>
--				reg_trace <= ss_disable_zero;
--			when reg_trace_ss_disable_char =>
--				reg_trace <= ss_disable_char;
--			when others =>
--				null;
--		end case;
-- end if;
...
--
-- L0068.reg_extend: .regfield 2 values same, zero, one default same
--
...
---- Start boilerplate code (use with utmost caution!)
-- update_reg_extend: process(clk, cpu_reg_extend)
-- begin
--	if (rising_edge(clk)) then
--		case cpu_reg_extend is
----			when reg_extend_same =>
----				reg_extend <= reg_extend;
...
--
-- L0071.sel_reg: .valfield 3 values zero, one, two, x, n, p default zero
--
...
--
-- L0072.reg_r: .regfield 3 values same, zero, r_plus_one, r_minus_one, yhi_rlo, rhi_ylo, b_t, -  default same
--
...
--
-- L0076.alu_r: .valfield 2 values t, d, b, reg_hi default t
--
...
--
-- L0077.alu_s: .valfield 2 values bus,  d, const, reg_lo default bus
--
...
--
-- L0087.alu_f: .valfield 3 values xor, and, ior, pass_r, r_plus_s, r_plus_ns, nr_plus_s, pass_s default xor
--
...
--
-- L0088.alu_cin: .valfield 1 values f1_or_f0, df default f1_or_f0
--
...
--
-- L0090.ct_oper: .valfield 4 values disable, enable default disable
--
