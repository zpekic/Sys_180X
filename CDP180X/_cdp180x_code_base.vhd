...
-- L0322@0079.XNOP: if false then next else NOP
--  bus_state = 0000, if (1111) then 00000000 else 10100101, reg_d <= 000, reg_df <= 00, reg_t <= 00, reg_b <= 00, reg_x <= 00, reg_p <= 00, reg_in <= 0, reg_q <= 00, reg_mie <= 00, reg_trace <= 00, reg_extend <= 00, sel_reg = 000, reg_r <= 000, alu_r = 00, alu_s = 00, alu_f = 000, alu_cin = 0, ct_oper = 0000;
121 => X"0" & X"F" & X"00" & X"A5" & O"0" & "00" & "00" & "00" & "00" & "00" & '0' & "00" & "00" & "00" & "00" & O"0" & O"0" & "00" & "00" & O"0" & '0' & X"0",
...
-- L0337@007D.INC: sel_reg = n, reg_r <= r_plus_one,if continue then fetch else dma_or_int
...
-- L0341@007E.DEC: sel_reg = n, reg_r <= r_minus_one,if continue then fetch else dma_or_int
...
-- L0345@007F.SBRANCH: bus_state = exec_memread, sel_reg = p, alu_f = pass_s, alu_s = bus, reg_b <= alu_y,if cond_3X then sbranch2 else next
...
-- L0349@0080.SKIP: sel_reg = p, reg_r <= r_plus_one,if continue then fetch else dma_or_int
...
-- L0352@0081.sbranch2: sel_reg = p, reg_r <= rhi_ylo, alu_f = pass_r, alu_r = b,if continue then fetch else dma_or_int
...
-- L0356@0082.LDA: bus_state = exec_memread, sel_reg = n, alu_f = pass_s, alu_s = bus, reg_d <= alu_y, reg_r <= r_plus_one,if continue then fetch else dma_or_int
...
-- L0360@0083.STR: bus_state = exec_memwrite, sel_reg = n, alu_f = pass_r, alu_r = d,if continue then fetch else dma_or_int
...
-- L0364@0084.OUT: bus_state = exec_iowrite, sel_reg = x, reg_r <= r_plus_one,if continue then fetch else dma_or_int
...
-- L0368@0085.IRX: sel_reg = x, reg_r <= r_plus_one,if continue then fetch else dma_or_int
...
-- L0372@0086.INP: bus_state = exec_ioread, sel_reg = x, alu_f = pass_s, alu_s = bus, reg_d <= alu_y,if continue then fetch else dma_or_int
...
-- L0376@0087.EXTEND: if mode_1805 then next else NOP
...
-- L0378@0088. bus_state = fetch_memread, sel_reg = p, reg_in <= alu_y, alu_f = pass_s, alu_s = bus, reg_extend <= one,reg_r <= r_plus_one,if false then next else load_b
...
-- L0383@0089.RET: alu_f = pass_r, alu_r = b, reg_x <= alu_yhi, reg_p <= alu_ylo, reg_mie <= enable,sel_reg = x, reg_r <= r_plus_one,if continue then fetch else dma_or_int
...
-- L0388@008A.DIS: alu_f = pass_r, alu_r = b, reg_x <= alu_yhi, reg_p <= alu_ylo, reg_mie <= disable,sel_reg = x, reg_r <= r_plus_one,if continue then fetch else dma_or_int
...
-- L0393@008B.LDXA: alu_f = pass_r, alu_r = b, reg_d <= alu_y
...
-- L0395@008C. sel_reg = x, reg_r <= r_plus_one,if continue then fetch else dma_or_int
--  bus_state = 0000, if (0110) then 00000100 else 00010000, reg_d <= 000, reg_df <= 00, reg_t <= 00, reg_b <= 00, reg_x <= 00, reg_p <= 00, reg_in <= 0, reg_q <= 00, reg_mie <= 00, reg_trace <= 00, reg_extend <= 00, sel_reg = 011, reg_r <= 010, alu_r = 00, alu_s = 00, alu_f = 000, alu_cin = 0, ct_oper = 0000;
140 => X"0" & X"6" & X"04" & X"10" & O"0" & "00" & "00" & "00" & "00" & "00" & '0' & "00" & "00" & "00" & "00" & O"3" & O"2" & "00" & "00" & O"0" & '0' & X"0",

-- L0399@008D.STXD: bus_state = exec_memwrite, alu_f = pass_r, alu_r = d, sel_reg = x
...
-- L0401@008E. sel_reg = x, reg_r <= r_minus_one,if continue then fetch else dma_or_int
...
-- L0406@008F.ADC: reg_df <= alu_cout, reg_d <= alu_y, alu_f = r_plus_s, alu_r = b, alu_s = d, alu_cin = df,if continue then fetch else dma_or_int
--  bus_state = 0000, if (0110) then 00000100 else 00010000, reg_d <= 001, reg_df <= 11, reg_t <= 00, reg_b <= 00, reg_x <= 00, reg_p <= 00, reg_in <= 0, reg_q <= 00, reg_mie <= 00, reg_trace <= 00, reg_extend <= 00, sel_reg = 000, reg_r <= 000, alu_r = 10, alu_s = 01, alu_f = 100, alu_cin = 1, ct_oper = 0000;
143 => X"0" & X"6" & X"04" & X"10" & O"1" & "11" & "00" & "00" & "00" & "00" & '0' & "00" & "00" & "00" & "00" & O"0" & O"0" & "10" & "01" & O"4" & '1' & X"0",

-- L0410@0090.SDB: reg_df <= alu_cout, reg_d <= alu_y, alu_f = r_plus_ns, alu_r = b, alu_s = d, alu_cin = df,if continue then fetch else dma_or_int
...
-- L0414@0091.SHRC: reg_d <= shift_dn_df, reg_df <= d_lsb,if continue then fetch else dma_or_int
...
-- L0419@0092.SMB: reg_df <= alu_cout, reg_d <= alu_y, alu_f = nr_plus_s, alu_r = b, alu_s = d, alu_cin = df,if continue then fetch else dma_or_int
--  bus_state = 0000, if (0110) then 00000100 else 00010000, reg_d <= 001, reg_df <= 11, reg_t <= 00, reg_b <= 00, reg_x <= 00, reg_p <= 00, reg_in <= 0, reg_q <= 00, reg_mie <= 00, reg_trace <= 00, reg_extend <= 00, sel_reg = 000, reg_r <= 000, alu_r = 10, alu_s = 01, alu_f = 110, alu_cin = 1, ct_oper = 0000;
146 => X"0" & X"6" & X"04" & X"10" & O"1" & "11" & "00" & "00" & "00" & "00" & '0' & "00" & "00" & "00" & "00" & O"0" & O"0" & "10" & "01" & O"6" & '1' & X"0",

-- L0423@0093.SAV: bus_state = exec_memwrite, sel_reg = x, alu_f = pass_r, alu_r = t,if continue then fetch else dma_or_int
...
-- L0429@0095. bus_state = exec_memwrite, reg_x <= p, sel_reg = two, alu_f = pass_r, alu_r = t, reg_r <= r_minus_one,if continue then fetch else dma_or_int
...
-- L0433@0096.REQ: reg_q <= zero,if continue then fetch else dma_or_int
...
-- L0437@0097.SEQ: reg_q <= one,if continue then fetch else dma_or_int
...
-- L0442@0098.ADCI: bus_state = exec_memread, sel_reg = p, reg_b <= alu_y, alu_f = pass_s, alu_s = bus, reg_r <= r_plus_one,if false then next else ADC
...
-- L0446@0099.SDBI: bus_state = exec_memread, sel_reg = p, reg_b <= alu_y, alu_f = pass_s, alu_s = bus, reg_r <= r_plus_one,if false then next else SDB
...
-- L0450@009A.SHLC: reg_df <= alu_cout, reg_d <= alu_y, alu_f = r_plus_s, alu_r = d, alu_s = d, alu_cin = df,if continue then fetch else dma_or_int
...
-- L0455@009B.SMBI: bus_state = exec_memread, sel_reg = p, reg_b <= alu_y, alu_f = pass_s, alu_s = bus, reg_r <= r_plus_one,if false then next else SMB
...
-- L0459@009C.GLO: reg_d <= alu_y, alu_f = pass_s, alu_s = reg_lo, sel_reg = n,if continue then fetch else dma_or_int
...
-- L0463@009D.GHI: reg_d <= alu_y, alu_f = pass_r, alu_r = reg_hi, sel_reg = n,if continue then fetch else dma_or_int
...
-- L0467@009E.PLO: reg_r <= rhi_ylo, alu_f = pass_r, alu_r = d, sel_reg = n,if continue then fetch else dma_or_int
...
-- L0471@009F.PHI: reg_r <= yhi_rlo, alu_f = pass_r, alu_r = d, sel_reg = n,if continue then fetch else dma_or_int
...
-- L0475@00A0.LBRANCH: bus_state = exec_memread, sel_reg = p, reg_b <= alu_y, alu_f = pass_s, alu_s = bus, reg_r <= r_plus_one,if cond_CX then next else SKIP
...
-- L0478@00A1. bus_state = exec_memread, sel_reg = p, reg_r <= rhi_ylo, alu_f = pass_s, alu_s = bus
...
-- L0480@00A2. sel_reg = p, reg_r <= yhi_rlo, alu_f = pass_r, alu_r = b,if continue then fetch else dma_or_int
...
-- L0490@00A3.LSKIP: if cond_CX then next else NOP
...
-- L0493@00A4.SKIP2: sel_reg = p, reg_r <= r_plus_one,if false then next else SKIP
...
-- L0497@00A5.NOP: if continue then fetch else dma_or_int
...
-- L0500@00A6.SEP: reg_p <= n,if continue then fetch else dma_or_int
...
-- L0504@00A7.SEX: reg_x <= n,if continue then fetch else dma_or_int
...
-- L0508@00A8.LDX: reg_d <= alu_y, alu_f = pass_r, alu_r = b,if continue then fetch else dma_or_int
...
-- L0512@00A9.OR: reg_d <= alu_y, alu_f = ior, alu_r = b, alu_s = d,if continue then fetch else dma_or_int
...
-- L0516@00AA.AND: reg_d <= alu_y, alu_f = and, alu_r = b, alu_s = d,if continue then fetch else dma_or_int
...
-- L0520@00AB.XOR: reg_d <= alu_y, alu_f = xor, alu_r = b, alu_s = d,if continue then fetch else dma_or_int
...
-- L0525@00AC.ADD: reg_df <= alu_cout, reg_d <= alu_y, alu_f = r_plus_s, alu_r = b, alu_s = d, alu_cin = f1_or_f0,if continue then fetch else dma_or_int
--  bus_state = 0000, if (0110) then 00000100 else 00010000, reg_d <= 001, reg_df <= 11, reg_t <= 00, reg_b <= 00, reg_x <= 00, reg_p <= 00, reg_in <= 0, reg_q <= 00, reg_mie <= 00, reg_trace <= 00, reg_extend <= 00, sel_reg = 000, reg_r <= 000, alu_r = 10, alu_s = 01, alu_f = 100, alu_cin = 0, ct_oper = 0000;
172 => X"0" & X"6" & X"04" & X"10" & O"1" & "11" & "00" & "00" & "00" & "00" & '0' & "00" & "00" & "00" & "00" & O"0" & O"0" & "10" & "01" & O"4" & '0' & X"0",

-- L0529@00AD.SD: reg_df <= alu_cout, reg_d <= alu_y, alu_f = r_plus_ns, alu_r = b, alu_s = d, alu_cin = f1_or_f0,if continue then fetch else dma_or_int
...
-- L0533@00AE.SHR: reg_d <= shift_dn_0, reg_df <= d_lsb,if continue then fetch else dma_or_int
...
-- L0538@00AF.SM: reg_df <= alu_cout, reg_d <= alu_y, alu_f = nr_plus_s, alu_r = b, alu_s = d, alu_cin = f1_or_f0,if continue then fetch else dma_or_int
--  bus_state = 0000, if (0110) then 00000100 else 00010000, reg_d <= 001, reg_df <= 11, reg_t <= 00, reg_b <= 00, reg_x <= 00, reg_p <= 00, reg_in <= 0, reg_q <= 00, reg_mie <= 00, reg_trace <= 00, reg_extend <= 00, sel_reg = 000, reg_r <= 000, alu_r = 10, alu_s = 01, alu_f = 110, alu_cin = 0, ct_oper = 0000;
175 => X"0" & X"6" & X"04" & X"10" & O"1" & "11" & "00" & "00" & "00" & "00" & '0' & "00" & "00" & "00" & "00" & O"0" & O"0" & "10" & "01" & O"6" & '0' & X"0",

-- L0542@00B0.LDI: bus_state = exec_memread, sel_reg = p, reg_d <= alu_y, alu_f = pass_s, alu_s = bus, reg_r <= r_plus_one,if continue then fetch else dma_or_int
...
-- L0546@00B1.ORI: bus_state = exec_memread, sel_reg = p, reg_b <= alu_y, alu_f = pass_s, alu_s = bus, reg_r <= r_plus_one,if false then next else OR
...
-- L0550@00B2.ANI: bus_state = exec_memread, sel_reg = p, reg_b <= alu_y, alu_f = pass_s, alu_s = bus, reg_r <= r_plus_one,if false then next else AND
...
-- L0554@00B3.XRI: bus_state = exec_memread, sel_reg = p, reg_b <= alu_y, alu_f = pass_s, alu_s = bus, reg_r <= r_plus_one,if false then next else XOR
...
-- L0559@00B4.ADI: bus_state = exec_memread, sel_reg = p, reg_b <= alu_y, alu_f = pass_s, alu_s = bus, reg_r <= r_plus_one,if false then next else ADD
...
-- L0563@00B5.SDI: bus_state = exec_memread, sel_reg = p, reg_b <= alu_y, alu_f = pass_s, alu_s = bus, reg_r <= r_plus_one,if false then next else SD
...
-- L0567@00B6.SHL: reg_df <= alu_cout, reg_d <= alu_y, alu_f = r_plus_s, alu_r = d, alu_s = d, alu_cin = f1_or_f0,if continue then fetch else dma_or_int
...
-- L0572@00B7.SMI: bus_state = exec_memread, sel_reg = p, reg_b <= alu_y, alu_f = pass_s, alu_s = bus, reg_r <= r_plus_one,if false then next else SM
...
-- L0579@00B8.STPC: sel_reg = n,if continue then fetch else dma_or_int
--  bus_state = 0000, if (0110) then 00000100 else 00010000, reg_d <= 000, reg_df <= 00, reg_t <= 00, reg_b <= 00, reg_x <= 00, reg_p <= 00, reg_in <= 0, reg_q <= 00, reg_mie <= 00, reg_trace <= 00, reg_extend <= 00, sel_reg = 100, reg_r <= 000, alu_r = 00, alu_s = 00, alu_f = 000, alu_cin = 0, ct_oper = 0000;
184 => X"0" & X"6" & X"04" & X"10" & O"0" & "00" & "00" & "00" & "00" & "00" & '0' & "00" & "00" & "00" & "00" & O"4" & O"0" & "00" & "00" & O"0" & '0' & X"0",

-- L0583@00B9.DTC: sel_reg = n,if continue then fetch else dma_or_int
--  bus_state = 0000, if (0110) then 00000100 else 00010000, reg_d <= 000, reg_df <= 00, reg_t <= 00, reg_b <= 00, reg_x <= 00, reg_p <= 00, reg_in <= 0, reg_q <= 00, reg_mie <= 00, reg_trace <= 00, reg_extend <= 00, sel_reg = 100, reg_r <= 000, alu_r = 00, alu_s = 00, alu_f = 000, alu_cin = 0, ct_oper = 0000;
185 => X"0" & X"6" & X"04" & X"10" & O"0" & "00" & "00" & "00" & "00" & "00" & '0' & "00" & "00" & "00" & "00" & O"4" & O"0" & "00" & "00" & O"0" & '0' & X"0",

-- L0587@00BA.SPM2: sel_reg = n,if continue then fetch else dma_or_int
--  bus_state = 0000, if (0110) then 00000100 else 00010000, reg_d <= 000, reg_df <= 00, reg_t <= 00, reg_b <= 00, reg_x <= 00, reg_p <= 00, reg_in <= 0, reg_q <= 00, reg_mie <= 00, reg_trace <= 00, reg_extend <= 00, sel_reg = 100, reg_r <= 000, alu_r = 00, alu_s = 00, alu_f = 000, alu_cin = 0, ct_oper = 0000;
186 => X"0" & X"6" & X"04" & X"10" & O"0" & "00" & "00" & "00" & "00" & "00" & '0' & "00" & "00" & "00" & "00" & O"4" & O"0" & "00" & "00" & O"0" & '0' & X"0",

-- L0591@00BB.SCM2: sel_reg = n,if continue then fetch else dma_or_int
--  bus_state = 0000, if (0110) then 00000100 else 00010000, reg_d <= 000, reg_df <= 00, reg_t <= 00, reg_b <= 00, reg_x <= 00, reg_p <= 00, reg_in <= 0, reg_q <= 00, reg_mie <= 00, reg_trace <= 00, reg_extend <= 00, sel_reg = 100, reg_r <= 000, alu_r = 00, alu_s = 00, alu_f = 000, alu_cin = 0, ct_oper = 0000;
187 => X"0" & X"6" & X"04" & X"10" & O"0" & "00" & "00" & "00" & "00" & "00" & '0' & "00" & "00" & "00" & "00" & O"4" & O"0" & "00" & "00" & O"0" & '0' & X"0",

-- L0595@00BC.SPM1: sel_reg = n,if continue then fetch else dma_or_int
--  bus_state = 0000, if (0110) then 00000100 else 00010000, reg_d <= 000, reg_df <= 00, reg_t <= 00, reg_b <= 00, reg_x <= 00, reg_p <= 00, reg_in <= 0, reg_q <= 00, reg_mie <= 00, reg_trace <= 00, reg_extend <= 00, sel_reg = 100, reg_r <= 000, alu_r = 00, alu_s = 00, alu_f = 000, alu_cin = 0, ct_oper = 0000;
188 => X"0" & X"6" & X"04" & X"10" & O"0" & "00" & "00" & "00" & "00" & "00" & '0' & "00" & "00" & "00" & "00" & O"4" & O"0" & "00" & "00" & O"0" & '0' & X"0",

-- L0599@00BD.SCM1: sel_reg = n,if continue then fetch else dma_or_int
--  bus_state = 0000, if (0110) then 00000100 else 00010000, reg_d <= 000, reg_df <= 00, reg_t <= 00, reg_b <= 00, reg_x <= 00, reg_p <= 00, reg_in <= 0, reg_q <= 00, reg_mie <= 00, reg_trace <= 00, reg_extend <= 00, sel_reg = 100, reg_r <= 000, alu_r = 00, alu_s = 00, alu_f = 000, alu_cin = 0, ct_oper = 0000;
189 => X"0" & X"6" & X"04" & X"10" & O"0" & "00" & "00" & "00" & "00" & "00" & '0' & "00" & "00" & "00" & "00" & O"4" & O"0" & "00" & "00" & O"0" & '0' & X"0",

-- L0603@00BE.LDC: sel_reg = n,if continue then fetch else dma_or_int
--  bus_state = 0000, if (0110) then 00000100 else 00010000, reg_d <= 000, reg_df <= 00, reg_t <= 00, reg_b <= 00, reg_x <= 00, reg_p <= 00, reg_in <= 0, reg_q <= 00, reg_mie <= 00, reg_trace <= 00, reg_extend <= 00, sel_reg = 100, reg_r <= 000, alu_r = 00, alu_s = 00, alu_f = 000, alu_cin = 0, ct_oper = 0000;
190 => X"0" & X"6" & X"04" & X"10" & O"0" & "00" & "00" & "00" & "00" & "00" & '0' & "00" & "00" & "00" & "00" & O"4" & O"0" & "00" & "00" & O"0" & '0' & X"0",

-- L0607@00BF.STM: sel_reg = n,if continue then fetch else dma_or_int
--  bus_state = 0000, if (0110) then 00000100 else 00010000, reg_d <= 000, reg_df <= 00, reg_t <= 00, reg_b <= 00, reg_x <= 00, reg_p <= 00, reg_in <= 0, reg_q <= 00, reg_mie <= 00, reg_trace <= 00, reg_extend <= 00, sel_reg = 100, reg_r <= 000, alu_r = 00, alu_s = 00, alu_f = 000, alu_cin = 0, ct_oper = 0000;
191 => X"0" & X"6" & X"04" & X"10" & O"0" & "00" & "00" & "00" & "00" & "00" & '0' & "00" & "00" & "00" & "00" & O"4" & O"0" & "00" & "00" & O"0" & '0' & X"0",

-- L0611@00C0.GEC: sel_reg = n,if continue then fetch else dma_or_int
--  bus_state = 0000, if (0110) then 00000100 else 00010000, reg_d <= 000, reg_df <= 00, reg_t <= 00, reg_b <= 00, reg_x <= 00, reg_p <= 00, reg_in <= 0, reg_q <= 00, reg_mie <= 00, reg_trace <= 00, reg_extend <= 00, sel_reg = 100, reg_r <= 000, alu_r = 00, alu_s = 00, alu_f = 000, alu_cin = 0, ct_oper = 0000;
192 => X"0" & X"6" & X"04" & X"10" & O"0" & "00" & "00" & "00" & "00" & "00" & '0' & "00" & "00" & "00" & "00" & O"4" & O"0" & "00" & "00" & O"0" & '0' & X"0",

-- L0615@00C1.ETQ: sel_reg = n,if continue then fetch else dma_or_int
--  bus_state = 0000, if (0110) then 00000100 else 00010000, reg_d <= 000, reg_df <= 00, reg_t <= 00, reg_b <= 00, reg_x <= 00, reg_p <= 00, reg_in <= 0, reg_q <= 00, reg_mie <= 00, reg_trace <= 00, reg_extend <= 00, sel_reg = 100, reg_r <= 000, alu_r = 00, alu_s = 00, alu_f = 000, alu_cin = 0, ct_oper = 0000;
193 => X"0" & X"6" & X"04" & X"10" & O"0" & "00" & "00" & "00" & "00" & "00" & '0' & "00" & "00" & "00" & "00" & O"4" & O"0" & "00" & "00" & O"0" & '0' & X"0",

-- L0619@00C2.XIE: sel_reg = n,if continue then fetch else dma_or_int
--  bus_state = 0000, if (0110) then 00000100 else 00010000, reg_d <= 000, reg_df <= 00, reg_t <= 00, reg_b <= 00, reg_x <= 00, reg_p <= 00, reg_in <= 0, reg_q <= 00, reg_mie <= 00, reg_trace <= 00, reg_extend <= 00, sel_reg = 100, reg_r <= 000, alu_r = 00, alu_s = 00, alu_f = 000, alu_cin = 0, ct_oper = 0000;
194 => X"0" & X"6" & X"04" & X"10" & O"0" & "00" & "00" & "00" & "00" & "00" & '0' & "00" & "00" & "00" & "00" & O"4" & O"0" & "00" & "00" & O"0" & '0' & X"0",

-- L0623@00C3.XID: sel_reg = n,if continue then fetch else dma_or_int
--  bus_state = 0000, if (0110) then 00000100 else 00010000, reg_d <= 000, reg_df <= 00, reg_t <= 00, reg_b <= 00, reg_x <= 00, reg_p <= 00, reg_in <= 0, reg_q <= 00, reg_mie <= 00, reg_trace <= 00, reg_extend <= 00, sel_reg = 100, reg_r <= 000, alu_r = 00, alu_s = 00, alu_f = 000, alu_cin = 0, ct_oper = 0000;
195 => X"0" & X"6" & X"04" & X"10" & O"0" & "00" & "00" & "00" & "00" & "00" & '0' & "00" & "00" & "00" & "00" & O"4" & O"0" & "00" & "00" & O"0" & '0' & X"0",

-- L0627@00C4.CIE: sel_reg = n,if continue then fetch else dma_or_int
--  bus_state = 0000, if (0110) then 00000100 else 00010000, reg_d <= 000, reg_df <= 00, reg_t <= 00, reg_b <= 00, reg_x <= 00, reg_p <= 00, reg_in <= 0, reg_q <= 00, reg_mie <= 00, reg_trace <= 00, reg_extend <= 00, sel_reg = 100, reg_r <= 000, alu_r = 00, alu_s = 00, alu_f = 000, alu_cin = 0, ct_oper = 0000;
196 => X"0" & X"6" & X"04" & X"10" & O"0" & "00" & "00" & "00" & "00" & "00" & '0' & "00" & "00" & "00" & "00" & O"4" & O"0" & "00" & "00" & O"0" & '0' & X"0",

-- L0631@00C5.CID: sel_reg = n,if continue then fetch else dma_or_int
--  bus_state = 0000, if (0110) then 00000100 else 00010000, reg_d <= 000, reg_df <= 00, reg_t <= 00, reg_b <= 00, reg_x <= 00, reg_p <= 00, reg_in <= 0, reg_q <= 00, reg_mie <= 00, reg_trace <= 00, reg_extend <= 00, sel_reg = 100, reg_r <= 000, alu_r = 00, alu_s = 00, alu_f = 000, alu_cin = 0, ct_oper = 0000;
197 => X"0" & X"6" & X"04" & X"10" & O"0" & "00" & "00" & "00" & "00" & "00" & '0' & "00" & "00" & "00" & "00" & O"4" & O"0" & "00" & "00" & O"0" & '0' & X"0",

-- L0635@00C6.DBNZ: sel_reg = n, reg_r <= r_minus_one,if alu16_zero then skip2 else next
--  bus_state = 0000, if (1011) then 11001010 else 00000000, reg_d <= 000, reg_df <= 00, reg_t <= 00, reg_b <= 00, reg_x <= 00, reg_p <= 00, reg_in <= 0, reg_q <= 00, reg_mie <= 00, reg_trace <= 00, reg_extend <= 00, sel_reg = 100, reg_r <= 011, alu_r = 00, alu_s = 00, alu_f = 000, alu_cin = 0, ct_oper = 0000;
198 => X"0" & X"B" & X"CA" & X"00" & O"0" & "00" & "00" & "00" & "00" & "00" & '0' & "00" & "00" & "00" & "00" & O"4" & O"3" & "00" & "00" & O"0" & '0' & X"0",

-- L0638@00C7. bus_state = exec_memread, sel_reg = p, reg_b <= alu_y, alu_f = pass_s, alu_s = bus, reg_r <= r_plus_one
...
-- L0640@00C8. bus_state = exec_memread, sel_reg = p, reg_r <= rhi_ylo, alu_f = pass_s, alu_s = bus
--  bus_state = 0001, if (0000) then 00000000 else 00000000, reg_d <= 000, reg_df <= 00, reg_t <= 00, reg_b <= 00, reg_x <= 00, reg_p <= 00, reg_in <= 0, reg_q <= 00, reg_mie <= 00, reg_trace <= 00, reg_extend <= 00, sel_reg = 101, reg_r <= 101, alu_r = 00, alu_s = 00, alu_f = 111, alu_cin = 0, ct_oper = 0000;
200 => X"1" & X"0" & X"00" & X"00" & O"0" & "00" & "00" & "00" & "00" & "00" & '0' & "00" & "00" & "00" & "00" & O"5" & O"5" & "00" & "00" & O"7" & '0' & X"0",

-- L0642@00C9. sel_reg = p, reg_r <= yhi_rlo, alu_f = pass_r, alu_r = b,if continue then fetch else dma_or_int
--  bus_state = 0000, if (0110) then 00000100 else 00010000, reg_d <= 000, reg_df <= 00, reg_t <= 00, reg_b <= 00, reg_x <= 00, reg_p <= 00, reg_in <= 0, reg_q <= 00, reg_mie <= 00, reg_trace <= 00, reg_extend <= 00, sel_reg = 101, reg_r <= 100, alu_r = 10, alu_s = 00, alu_f = 011, alu_cin = 0, ct_oper = 0000;
201 => X"0" & X"6" & X"04" & X"10" & O"0" & "00" & "00" & "00" & "00" & "00" & '0' & "00" & "00" & "00" & "00" & O"5" & O"4" & "10" & "00" & O"3" & '0' & X"0",

-- L0645@00CA.skip2: sel_reg = p, reg_r <= r_plus_one,if false then next else SKIP
--  bus_state = 0000, if (1111) then 00000000 else 10000000, reg_d <= 000, reg_df <= 00, reg_t <= 00, reg_b <= 00, reg_x <= 00, reg_p <= 00, reg_in <= 0, reg_q <= 00, reg_mie <= 00, reg_trace <= 00, reg_extend <= 00, sel_reg = 101, reg_r <= 010, alu_r = 00, alu_s = 00, alu_f = 000, alu_cin = 0, ct_oper = 0000;
202 => X"0" & X"F" & X"00" & X"80" & O"0" & "00" & "00" & "00" & "00" & "00" & '0' & "00" & "00" & "00" & "00" & O"5" & O"2" & "00" & "00" & O"0" & '0' & X"0",

-- L0649@00CB.BCI: bus_state = exec_memread, sel_reg = p, alu_f = pass_s, alu_s = bus, reg_b <= alu_y,if counterInt then sbranch2 else SKIP
--  bus_state = 0001, if (1010) then 10000001 else 10000000, reg_d <= 000, reg_df <= 00, reg_t <= 00, reg_b <= 01, reg_x <= 00, reg_p <= 00, reg_in <= 0, reg_q <= 00, reg_mie <= 00, reg_trace <= 00, reg_extend <= 00, sel_reg = 101, reg_r <= 000, alu_r = 00, alu_s = 00, alu_f = 111, alu_cin = 0, ct_oper = 0000;
203 => X"1" & X"A" & X"81" & X"80" & O"0" & "00" & "00" & "01" & "00" & "00" & '0' & "00" & "00" & "00" & "00" & O"5" & O"0" & "00" & "00" & O"7" & '0' & X"0",

-- L0653@00CC.BXI: bus_state = exec_memread, sel_reg = p, alu_f = pass_s, alu_s = bus, reg_b <= alu_y,if externalInt then sbranch2 else SKIP
--  bus_state = 0001, if (1001) then 10000001 else 10000000, reg_d <= 000, reg_df <= 00, reg_t <= 00, reg_b <= 01, reg_x <= 00, reg_p <= 00, reg_in <= 0, reg_q <= 00, reg_mie <= 00, reg_trace <= 00, reg_extend <= 00, sel_reg = 101, reg_r <= 000, alu_r = 00, alu_s = 00, alu_f = 111, alu_cin = 0, ct_oper = 0000;
204 => X"1" & X"9" & X"81" & X"80" & O"0" & "00" & "00" & "01" & "00" & "00" & '0' & "00" & "00" & "00" & "00" & O"5" & O"0" & "00" & "00" & O"7" & '0' & X"0",

-- L0657@00CD.RLXA: bus_state = exec_memread, sel_reg = x, reg_b <= alu_y, alu_f = pass_s, alu_s = bus, reg_r <= r_plus_one
--  bus_state = 0001, if (0000) then 00000000 else 00000000, reg_d <= 000, reg_df <= 00, reg_t <= 00, reg_b <= 01, reg_x <= 00, reg_p <= 00, reg_in <= 0, reg_q <= 00, reg_mie <= 00, reg_trace <= 00, reg_extend <= 00, sel_reg = 011, reg_r <= 010, alu_r = 00, alu_s = 00, alu_f = 111, alu_cin = 0, ct_oper = 0000;
205 => X"1" & X"0" & X"00" & X"00" & O"0" & "00" & "00" & "01" & "00" & "00" & '0' & "00" & "00" & "00" & "00" & O"3" & O"2" & "00" & "00" & O"7" & '0' & X"0",

-- L0659@00CE. bus_state = exec_memread, sel_reg = x, reg_t <= alu_y, alu_f = pass_s, alu_s = bus, reg_r <= r_plus_one
...
-- L0661@00CF. sel_reg = n, reg_r <= yhi_rlo, alu_f = pass_r, alu_r = b, reg_b <= t
...
-- L0663@00D0. sel_reg = n, reg_r <= rhi_ylo, alu_f = pass_r, alu_r = b,if continue then fetch else dma_or_int
...
-- L0667@00D1.DSAV: sel_reg = x, reg_r <= r_minus_one
--  bus_state = 0000, if (0000) then 00000000 else 00000000, reg_d <= 000, reg_df <= 00, reg_t <= 00, reg_b <= 00, reg_x <= 00, reg_p <= 00, reg_in <= 0, reg_q <= 00, reg_mie <= 00, reg_trace <= 00, reg_extend <= 00, sel_reg = 011, reg_r <= 011, alu_r = 00, alu_s = 00, alu_f = 000, alu_cin = 0, ct_oper = 0000;
209 => X"0" & X"0" & X"00" & X"00" & O"0" & "00" & "00" & "00" & "00" & "00" & '0' & "00" & "00" & "00" & "00" & O"3" & O"3" & "00" & "00" & O"0" & '0' & X"0",

-- L0669@00D2. bus_state = exec_memwrite, sel_reg = x, alu_f = pass_r, alu_r = t, reg_r <= r_minus_one
...
-- L0671@00D3. bus_state = exec_memwrite, sel_reg = x, alu_f = pass_r, alu_r = d, reg_r <= r_minus_one, reg_b <= df_d_dn
...
-- L0673@00D4. bus_state = exec_memwrite, sel_reg = x, alu_f = pass_r, alu_r = b, reg_r <= r_minus_one,if continue then fetch else dma_or_int
...
-- L0677@00D5.SCAL: sel_reg = n, reg_t <= alu_y, alu_f = pass_s, alu_s = reg_lo
--  bus_state = 0000, if (0000) then 00000000 else 00000000, reg_d <= 000, reg_df <= 00, reg_t <= 01, reg_b <= 00, reg_x <= 00, reg_p <= 00, reg_in <= 0, reg_q <= 00, reg_mie <= 00, reg_trace <= 00, reg_extend <= 00, sel_reg = 100, reg_r <= 000, alu_r = 00, alu_s = 11, alu_f = 111, alu_cin = 0, ct_oper = 0000;
213 => X"0" & X"0" & X"00" & X"00" & O"0" & "00" & "01" & "00" & "00" & "00" & '0' & "00" & "00" & "00" & "00" & O"4" & O"0" & "00" & "11" & O"7" & '0' & X"0",

-- L0679@00D6. sel_reg = n, reg_b <= alu_y, alu_f = pass_r, alu_r = reg_hi
...
-- L0681@00D7. bus_state = exec_memwrite, sel_reg = x, alu_f = pass_r, alu_r = t, reg_r <= r_minus_one
--  bus_state = 0010, if (0000) then 00000000 else 00000000, reg_d <= 000, reg_df <= 00, reg_t <= 00, reg_b <= 00, reg_x <= 00, reg_p <= 00, reg_in <= 0, reg_q <= 00, reg_mie <= 00, reg_trace <= 00, reg_extend <= 00, sel_reg = 011, reg_r <= 011, alu_r = 00, alu_s = 00, alu_f = 011, alu_cin = 0, ct_oper = 0000;
215 => X"2" & X"0" & X"00" & X"00" & O"0" & "00" & "00" & "00" & "00" & "00" & '0' & "00" & "00" & "00" & "00" & O"3" & O"3" & "00" & "00" & O"3" & '0' & X"0",

-- L0683@00D8. bus_state = exec_memwrite, sel_reg = x, alu_f = pass_r, alu_r = b, reg_r <= r_minus_one
...
-- L0685@00D9. sel_reg = p, reg_t <= alu_y, alu_f = pass_s, alu_s = reg_lo
...
-- L0687@00DA. sel_reg = p, reg_b <= alu_y, alu_f = pass_r, alu_r = reg_hi
...
-- L0689@00DB. sel_reg = n, reg_r <= b_t
...
-- L0691@00DC. bus_state = exec_memread, sel_reg = n, reg_t <= alu_y, alu_f = pass_s, alu_s = bus, reg_r <= r_plus_one
...
-- L0693@00DD. bus_state = exec_memread, sel_reg = n, reg_b <= alu_y, alu_f = pass_s, alu_s = bus, reg_r <= r_plus_one
...
-- L0695@00DE. sel_reg = p, reg_r <= b_t,if continue then fetch else dma_or_int
...
-- L0699@00DF.SRET: sel_reg = n, reg_t <= alu_y, alu_f = pass_s, alu_s = reg_lo
--  bus_state = 0000, if (0000) then 00000000 else 00000000, reg_d <= 000, reg_df <= 00, reg_t <= 01, reg_b <= 00, reg_x <= 00, reg_p <= 00, reg_in <= 0, reg_q <= 00, reg_mie <= 00, reg_trace <= 00, reg_extend <= 00, sel_reg = 100, reg_r <= 000, alu_r = 00, alu_s = 11, alu_f = 111, alu_cin = 0, ct_oper = 0000;
223 => X"0" & X"0" & X"00" & X"00" & O"0" & "00" & "01" & "00" & "00" & "00" & '0' & "00" & "00" & "00" & "00" & O"4" & O"0" & "00" & "11" & O"7" & '0' & X"0",

-- L0701@00E0. sel_reg = n, reg_b <= alu_y, alu_f = pass_r, alu_r = reg_hi
--  bus_state = 0000, if (0000) then 00000000 else 00000000, reg_d <= 000, reg_df <= 00, reg_t <= 00, reg_b <= 01, reg_x <= 00, reg_p <= 00, reg_in <= 0, reg_q <= 00, reg_mie <= 00, reg_trace <= 00, reg_extend <= 00, sel_reg = 100, reg_r <= 000, alu_r = 11, alu_s = 00, alu_f = 011, alu_cin = 0, ct_oper = 0000;
224 => X"0" & X"0" & X"00" & X"00" & O"0" & "00" & "00" & "01" & "00" & "00" & '0' & "00" & "00" & "00" & "00" & O"4" & O"0" & "11" & "00" & O"3" & '0' & X"0",

-- L0703@00E1. sel_reg = x, reg_r <= r_plus_one
...
-- L0705@00E2. sel_reg = p, reg_r <= b_t
...
-- L0707@00E3. bus_state = exec_memread, sel_reg = x, reg_t <= alu_y, alu_f = pass_s, alu_s = bus, reg_r <= r_plus_one
--  bus_state = 0001, if (0000) then 00000000 else 00000000, reg_d <= 000, reg_df <= 00, reg_t <= 01, reg_b <= 00, reg_x <= 00, reg_p <= 00, reg_in <= 0, reg_q <= 00, reg_mie <= 00, reg_trace <= 00, reg_extend <= 00, sel_reg = 011, reg_r <= 010, alu_r = 00, alu_s = 00, alu_f = 111, alu_cin = 0, ct_oper = 0000;
227 => X"1" & X"0" & X"00" & X"00" & O"0" & "00" & "01" & "00" & "00" & "00" & '0' & "00" & "00" & "00" & "00" & O"3" & O"2" & "00" & "00" & O"7" & '0' & X"0",

-- L0709@00E4. bus_state = exec_memread, sel_reg = x, reg_b <= alu_y, alu_f = pass_s, alu_s = bus,if false then next else load_n
...
-- L0713@00E5.RSXD: sel_reg = n, reg_t <= alu_y, alu_f = pass_s, alu_s = reg_lo
--  bus_state = 0000, if (0000) then 00000000 else 00000000, reg_d <= 000, reg_df <= 00, reg_t <= 01, reg_b <= 00, reg_x <= 00, reg_p <= 00, reg_in <= 0, reg_q <= 00, reg_mie <= 00, reg_trace <= 00, reg_extend <= 00, sel_reg = 100, reg_r <= 000, alu_r = 00, alu_s = 11, alu_f = 111, alu_cin = 0, ct_oper = 0000;
229 => X"0" & X"0" & X"00" & X"00" & O"0" & "00" & "01" & "00" & "00" & "00" & '0' & "00" & "00" & "00" & "00" & O"4" & O"0" & "00" & "11" & O"7" & '0' & X"0",

-- L0715@00E6. sel_reg = n, reg_b <= alu_y, alu_f = pass_r, alu_r = reg_hi
--  bus_state = 0000, if (0000) then 00000000 else 00000000, reg_d <= 000, reg_df <= 00, reg_t <= 00, reg_b <= 01, reg_x <= 00, reg_p <= 00, reg_in <= 0, reg_q <= 00, reg_mie <= 00, reg_trace <= 00, reg_extend <= 00, sel_reg = 100, reg_r <= 000, alu_r = 11, alu_s = 00, alu_f = 011, alu_cin = 0, ct_oper = 0000;
230 => X"0" & X"0" & X"00" & X"00" & O"0" & "00" & "00" & "01" & "00" & "00" & '0' & "00" & "00" & "00" & "00" & O"4" & O"0" & "11" & "00" & O"3" & '0' & X"0",

-- L0717@00E7. bus_state = exec_memwrite, sel_reg = x, alu_f = pass_r, alu_r = t, reg_r <= r_minus_one
--  bus_state = 0010, if (0000) then 00000000 else 00000000, reg_d <= 000, reg_df <= 00, reg_t <= 00, reg_b <= 00, reg_x <= 00, reg_p <= 00, reg_in <= 0, reg_q <= 00, reg_mie <= 00, reg_trace <= 00, reg_extend <= 00, sel_reg = 011, reg_r <= 011, alu_r = 00, alu_s = 00, alu_f = 011, alu_cin = 0, ct_oper = 0000;
231 => X"2" & X"0" & X"00" & X"00" & O"0" & "00" & "00" & "00" & "00" & "00" & '0' & "00" & "00" & "00" & "00" & O"3" & O"3" & "00" & "00" & O"3" & '0' & X"0",

-- L0719@00E8. bus_state = exec_memwrite, sel_reg = x, alu_f = pass_r, alu_r = b, reg_r <= r_minus_one,if continue then fetch else dma_or_int
--  bus_state = 0010, if (0110) then 00000100 else 00010000, reg_d <= 000, reg_df <= 00, reg_t <= 00, reg_b <= 00, reg_x <= 00, reg_p <= 00, reg_in <= 0, reg_q <= 00, reg_mie <= 00, reg_trace <= 00, reg_extend <= 00, sel_reg = 011, reg_r <= 011, alu_r = 10, alu_s = 00, alu_f = 011, alu_cin = 0, ct_oper = 0000;
232 => X"2" & X"6" & X"04" & X"10" & O"0" & "00" & "00" & "00" & "00" & "00" & '0' & "00" & "00" & "00" & "00" & O"3" & O"3" & "10" & "00" & O"3" & '0' & X"0",

-- L0723@00E9.RNX: sel_reg = n, reg_t <= alu_y, alu_f = pass_s, alu_s = reg_lo
--  bus_state = 0000, if (0000) then 00000000 else 00000000, reg_d <= 000, reg_df <= 00, reg_t <= 01, reg_b <= 00, reg_x <= 00, reg_p <= 00, reg_in <= 0, reg_q <= 00, reg_mie <= 00, reg_trace <= 00, reg_extend <= 00, sel_reg = 100, reg_r <= 000, alu_r = 00, alu_s = 11, alu_f = 111, alu_cin = 0, ct_oper = 0000;
233 => X"0" & X"0" & X"00" & X"00" & O"0" & "00" & "01" & "00" & "00" & "00" & '0' & "00" & "00" & "00" & "00" & O"4" & O"0" & "00" & "11" & O"7" & '0' & X"0",

-- L0725@00EA. sel_reg = n, reg_b <= alu_y, alu_f = pass_r, alu_r = reg_hi
--  bus_state = 0000, if (0000) then 00000000 else 00000000, reg_d <= 000, reg_df <= 00, reg_t <= 00, reg_b <= 01, reg_x <= 00, reg_p <= 00, reg_in <= 0, reg_q <= 00, reg_mie <= 00, reg_trace <= 00, reg_extend <= 00, sel_reg = 100, reg_r <= 000, alu_r = 11, alu_s = 00, alu_f = 011, alu_cin = 0, ct_oper = 0000;
234 => X"0" & X"0" & X"00" & X"00" & O"0" & "00" & "00" & "01" & "00" & "00" & '0' & "00" & "00" & "00" & "00" & O"4" & O"0" & "11" & "00" & O"3" & '0' & X"0",

-- L0727@00EB. sel_reg = x, reg_r <= b_t,if continue then fetch else dma_or_int
...
-- L0731@00EC.RLDI: bus_state = exec_memread, sel_reg = p, reg_b <= alu_y, alu_f = pass_s, alu_s = bus, reg_r <= r_plus_one
--  bus_state = 0001, if (0000) then 00000000 else 00000000, reg_d <= 000, reg_df <= 00, reg_t <= 00, reg_b <= 01, reg_x <= 00, reg_p <= 00, reg_in <= 0, reg_q <= 00, reg_mie <= 00, reg_trace <= 00, reg_extend <= 00, sel_reg = 101, reg_r <= 010, alu_r = 00, alu_s = 00, alu_f = 111, alu_cin = 0, ct_oper = 0000;
236 => X"1" & X"0" & X"00" & X"00" & O"0" & "00" & "00" & "01" & "00" & "00" & '0' & "00" & "00" & "00" & "00" & O"5" & O"2" & "00" & "00" & O"7" & '0' & X"0",

-- L0733@00ED. bus_state = exec_memread, sel_reg = p, reg_t <= alu_y, alu_f = pass_s, alu_s = bus, reg_r <= r_plus_one
...
-- L0735@00EE.load_n: sel_reg = n, reg_r <= b_t,if continue then fetch else dma_or_int
